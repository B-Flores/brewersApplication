...
\
\
**The probability of that the starting pitcher outperforms the relief pitcher on a per-inning basis is **
**about 9.29%. The odds are therefore 0.102/1.** 
...
\
\
We are given 3 conditional probabilities. Their complements are easily determined as well:
\
\
...
\
\
The question asks us to calculate the following:
\
\
$$P(INSZ|FB\cap BALL)$$
\
\
We do this by calculated the proportion of events "INSZ" that are included within both "FB" and "BALL" out of the total amount of events that include both "FB" and "BALL." This gives us:
...
\
\
**Under this model, the pitcher with K% = 0.2 and BB% = 0.1 will outperform the pitcher with K% = 0.25 and BB% =** **0.20 approximately 66.38% of the time.**
\
\
...
#Evaluate marginal effect of K% on P(ERA < 4.00) for each pitcher
fun(0.2, 0.1)
fun(0.25, 0.2)
```
\
...
\
\
**I would prefer the linear model as general purpose tool in determining the quality of a pitcher. Although** **classifying pitchers into different categories can be useful, I don't think separating pitchers into 2** 
**categories is generally helpful. For example in this problem, pitchers are classified by the model** **projecting an ERA of lesser or greater than 4. This exercise could be helpful in filtering out potential** **candidates, but there is also a significant difference in production from a 4.00 ERA pitcher and a** 
...
# This simulation assumes that the braves have 9 games left and the mets have 8 games left. The probability of the braves winning the division is simulating using randomly distributed binomial variables defined by the values above and evaluated logically as "braves_win_div" given each case that this is possible. The results are summed up and averaged over 1 million iterations and returned as a percentage.
...
**in the outfield. This would include: Identities of all players (specifically outfielders; possibly **
**batters and pitchers), temperature, crowd size, batted ball launch characteristics, outfielder** 
...
**in the model, and so the model would likely need to be updated with either additional data or possibly** 
...
**with these goals in mind would affect my answer as well as whether there are other options at each** **infielder's respective position. Also given both Players A and B, it seems likely that Player B would** 
**be a veteran major league player given the projected decline in WAR/S over the contract length and that** **Player A is relatively young since they are projected to remain stable over the 4 year contract. Over** 
...
**and that both Players A and B would be a significant upgrade at their given position. The decision seems** **fairly clear that Player B would likely be worth the investment as the projected additional 2 WAR above** **Player A over the first season would align more with the goals and would more likely push the **
**organization over the line in a tight race. Now suppose the division is more competitive, and although** **winning is always the end goal, the organization is more focused on developing talent over the next** 
**couple of years. This would be an indicator that Player A's contract could be more beneficial in the **
**long run if saving money while remaining relatively competitive at their position is likely.  **
